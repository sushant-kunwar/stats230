...
*Plot 1: This scatter plot shows the relationship between total expenditure on instruction and average math score in 1992. There appears to be a positive correlation, with higher expenditures generally associated with higher average math scores, though the relationship is not perfectly linear. Specifically, most of the data points are concentrated in the upperleft of the plot, further hinting at the fact that this relationship is not linear.*

*Plot 2: This scatter plot displays the same variables as the previous one, but for the year 2015. Here, there is a similar issue where the data does not appear linear and is concentrated in the upper left of the plot. This hints that we should probably use the proportional expenditure on instruction rather than just pure expenditure on instruction.*
...
*Plot 3: This scatter plot displays the relationship between the proportional expenditure on instruction and average math score in 1992. Like the plot displaying the total expenditure, there is a positive correlation, but in this graph it is much more pronounced and is more linear. Here we can see that when a higher proportion of the budget goes to instruction, we would expect that the average math score would increase.*
...
```
Histogram 1 (1992): The histogram displays a distinct peak just below .2, indicating a concentration of bootstrapped correlation values around that point. The parametric confidence interval (teal line) appears narrower compared to the bootstrap confidence interval (green line).
